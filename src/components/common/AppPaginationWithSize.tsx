...
export const AppPaginationWithSize = ({
  totalCount,
  pageSize,
  onChange,
  page,
    ...
    (e: ChangeEvent<HTMLInputElement>) => {
      ...
      if (newSize * page > totalCount) {
        return onChange((prev) => ({
          ...prev,
          _limit: newSize,
          _page: Math.ceil(totalCount / newSize),
        }));
      }
  ...
  const availableSizes = useMemo(() => {
    return DEFAULT_SIZE_OPTIONS.filter(
      (size, i) => i === 0 || size <= totalCount
    );
  }, [totalCount]);

  const pageNums = useMemo<(number | string)[]>(() => {
    const count = totalCount > 0 ? Math.ceil(totalCount / pageSize) : 0;
    const arr: Array<number | string> = new Array(count)
      .fill(null)
      .map((_, i) => i + 1);
    ...
    const mainNums =
      page + 4 > arr.length
        ...
        : page - 2 > 0
        ? arr.slice(page - 2, page + 2)
        : arr.slice(1, 5);
    return [arr[0]]
      .concat(Math.abs(+arr[0] - +mainNums[0]) > 1 ? "..." : [])
      .concat(mainNums)
      .concat(
        Math.abs(+arr[arr.length - 1] - +mainNums[mainNums.length - 1]) > 1
          ? "..."
          : []
      )
    ...
    <FlexBox alignItems={"center"} flexDirection={{ xs: "column", md: "row" }}>
      <FormControl sx={{ mb: { xs: 1, md: 0 } }}>
        <TextField
          ...
          color={"primary"}
          value={
            availableSizes.includes(pageSize) ? pageSize : availableSizes[0]
          }
          size={"small"}
          sx={{ borderRadius: 3 }}
        >
      ...
      <FlexBox ml={1} alignItems={"center"}>
        ...
        {pageNums.map((num, i) =>
          !isNaN(+num) ? (
            ...
            >
              <Typography
                color={num === page ? "primary" : "default"}
                fontWeight={num === page ? 600 : 200}
              >
                {num}
              </Typography>
            </Box>
          ) : (
            <Typography key={`${i}...`} sx={{ md: 0.5 }}>
              ...
            </Typography>
          )
        )}
        <IconButton
          ...
          onClick={() =>
            handlePageChange(
              page + 1 <= pageNums[pageNums.length - 1] ? page + 1 : page
            )
          }
        >
          <KeyboardArrowRight />
